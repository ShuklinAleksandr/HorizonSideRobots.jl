...
module SituationDatas
    ...
    # ~~ ЦВЕТА ~~ 
    const BORDER_COLOR = :"#013220" # цвет границ
    const BODY_COLOR = :"#c72fff" # цвет тела робота
    const COLOR_ON_CLICK = :"#c2d6d3" # цвет тела робота при нажатии на него
    const MARKER_COLOR = :"#ffffff" # цвет маркера
    const PLOT_COLOR = :"#410eb7" # цвет внешнего поля
    const INNER_COLOR = :"#adff2f" # цвет внутреннего 
    const GRID_COLOR = :"#0e84b7" # цвет координатных линий
...
"""
            ...
            rcParams["axes.facecolor"] = INNER_COLOR # меняет цвет внутреннего поля

            rcParams["toolbar"]="None" # - строки toolbar в figure быть не должно
            rcParams["axes.edgecolor"]= PLOT_COLOR # - рамка осей должен не должна быть видимой
            rcParams["xtick.color"]= PLOT_COLOR # - разметка осей не должна быть видимой
            rcParams["ytick.color"]= PLOT_COLOR 
            rcParams["figure.figsize"]=(7*axes_size[1]/axes_size[2],7-0.2) # - размеры окна задаются с учетом отсутствия toolbar и сучетом фактических размеров клеточного поля (если бы имелся toolbar, то по умолчанию размеры canvas были бы - 7*7 дюймов)
            
            if newfig==true
                figure(facecolor = PLOT_COLOR) # - создается новое окно (пока без координатных осей)
            else
            ...
            grid(true, color = GRID_COLOR) # - отображаются координатные линии
            return nothing
        ...
        elseif Δx >= abs(Δy) && Δx >= ρ   
            ...
            if BUFF_SITUATION.robot_position == position 
                if IS_FIXED_ROBOT_POSITION == false                   
                    ...
                    body_create(BUFF_SITUATION.coefficient, floor(x)+0.5,floor(y)+0.5; body_color=COLOR_ON_CLICK)
